  ...
  <div style="margin-bottom: 10px;">
    ...
    >
          ...
          <el-popover trigger="hover" placement="top">
            ...
            <div slot="reference" class="name-wrapper">
              <el-button type="text" style="color: #215DA9" @click="toExpertDetail(scope.row)">
                {{ scope.row.properties.personName.split(';').slice(-1)[0]?scope.row.properties.personName.split(';').slice(-1)[0]:scope.row.properties.personName }}
              </el-button>
            </div>
          </el-popover>
        </template>
      ...
      <el-table-column label="所在地" align="left" prop="author" width="100">
        <template slot-scope="scope">
          {{ scope.row.properties.address?scope.row.properties.address:scope.row.properties.province }}
        </template>
      </el-table-column>
      <el-table-column label="研究领域/万方ID" align="left" prop="author" width="220">
        <template slot-scope="scope">
          {{ scope.row.properties.researchField?scope.row.properties.researchField:scope.row.properties._ID }}
        </template>
      </el-table-column>
      <el-table-column label="单位" align="left" prop="org" width="250">
        <template slot-scope="scope">
          {{ scope.row.properties.unitName?scope.row.properties.unitName:scope.row.properties.org }}
        </template>
      </el-table-column>
    </el-table>
