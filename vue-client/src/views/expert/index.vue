<template>
  <div class="my-dashboard-container">
    <!-- <HeadMenu @typeChange="typeChange"/> -->
    <el-container>
        ...
        <el-main style="padding:0 20px 20px 20px;">
          <el-card>
            <div style="display: flex;flex-direction: row;">
              <head-menu :content="content" style="width: 600px;" @onUpdateSearch="onUpdateSearch" @onUpdateContent="onUpdateContent" @typeChange="typeChange" />
          ...
          <el-radio-group v-model="radio">
            <el-radio label="related">最相关</el-radio>
            <el-radio label="latest">最新</el-radio>
            &lt;!&ndash;<el-radio label="popular">最热</el-radio>&ndash;&gt;
          </el-radio-group>
        </el-card>-->
        ...
        <el-table-column label="姓名" align="left" prop="title" width="150">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <span>{{ scope.row.properties.personName }}</span>
              <div slot="reference" class="name-wrapper">
                <el-button type="text" style="color: #215DA9" @click="toExpertDetail(scope.row)">
                  {{ scope.row.properties.personName?scope.row.properties.personName:scope.row.properties.name }}
                </el-button>
              </div>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column label="城市群" align="center" width="200">
          <template slot-scope="scope">
            <span>{{ scope.row.properties.urbanCluster }}</span>
          </template>
        </el-table-column>
        <!-- <el-table-column label="工作单位/职称" align="left" prop="author" width="180">
          <template slot-scope="scope">{{ scope.row.properties.unitName?scope.row.properties.unitName:scope.row.properties.professionName }}</template>
        </el-table-column> -->
        <el-table-column label="所在地" align="left" prop="author" width="100">
          <template slot-scope="scope">
            {{ scope.row.properties.address }}
          </template>
        </el-table-column>
        <el-table-column label="研究领域" align="left" prop="author" width="250">
          <template slot-scope="scope">
            {{ scope.row.properties.researchField }}
          </template>
        </el-table-column>
      </el-table>
    </el-drawer>
...
export default {
  ...
  data() {
    return {
      ...
      reknType: this.$route.query.reknType || 'name',
      sortField: '',
      listLoading: false,
      searchResult: [],
      patentListMap: [],
  ...
  watch: {
    searchRadio() {
      switch (this.searchRadio) {
        case 'list': { this.pageNumber = 1; this.onSearchData(); break }
  ...
  methods: {
    ...
    typeChange(reknType) {
      console.log('reknType发生了变化：：', reknType)
    ...
    onUpdateSearch(content) {
      ...
      if (this.category === '') {
        this.onSearchData()
      } else if (this.category === 'wf_mds_auth_new') {
        this.onSearch()
      } else {
    ...
    async onUpdateCategory(category) {
      ...
      if (this.category === '') {
        ...
        if (this.searchRadio === 'list') {
          if (this.category === 'wf_mds_auth_new') {
            this.onSearch()
          } else {
            this.onSubmit()
          }
    ...
    onSearchData() {
      ...
      if (this.reknType[0] === 'name') {
        params2['PersonName'] = this.content
      }
      if (this.reknType[0] === 'researchField') {
        params2['ResearchField'] = this.content
      }
      console.log('params2:', params2)
      knowledgeService.queryNbExpert(params2).then(res => {
        ...
        // 加入知识图谱
        knowledgeService.nbExpert2kg(res).then(res => {
          console.log('已加入资源图谱~')
        })
      }, err => {
    ...
    async onSubmit() {
      console.log('this.reknType', this.reknType)
      // 专家图谱
      this.searchResult = []
      // this.pageNumber = 0
      this.listLoading = true
      ...
      var H = {
        label: '专家',
        skip: (Number(this.pageNumber - 1)) * 20,
        limit: 20,
        urbanCluster: this.category
        // name: this.content
      }
      H[this.reknType] = this.content
      console.log('代理机构请求参数', H)
      var params1 = {
    ...
    onSearch() {
      ...
      knowledgeService.queryWf(params1).then(res => {
        ...
        _this.wrapRes = []
        _this.searchResult.forEach(element => {
          var wrapProp = {}
          wrapProp['properties'] = element._source
          wrapProp['properties']['personName'] = element._source.cname
          wrapProp['properties']['address'] = element._source.addr
          wrapProp['properties']['researchField'] = element._source.direction
          _this.wrapRes.push(wrapProp)
        })
        console.log('wrapRes:', _this.wrapRes)
        ...
        _this.listLoading = false
        // 加入知识图谱
        knowledgeService.wfScholar2kg(res).then(res => {
          console.log('已加入资源图谱~')
        })
      }, err => {
        console.error('请求失败：', err)
    ...
    async onSearchMap() {
      ...
      window.H[this.reknType] = this.content
      var params2 = {
    ...
    async onSubmit3() {
      ...
      window.H = {
        ...
        urbanCluster: this.category,
        skip: (Number(this.pageNumberMap - 1)) * 10,
        limit: 10
      }
      var params3 = {
        data: JSON.stringify(window.H)
    ...
    async current_changeMapDrawer(currentPage) {
      this.pageNumberMap = currentPage
      console.log(currentPage)
