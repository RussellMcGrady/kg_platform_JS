            ...
            <el-card shadow="never">
              ...
              <div v-if="stepActive===1">
                ...
                <el-row type="flex" style="margin-top: 60px" justify="center">
                  ...
                  <!-- <el-col :span="6">
                    <el-button type="text" @click="toTemplate('dir=mainStructure&name=输入4 独立性评价输入-模板.xlsx')">下载独立性评价模板<i class="el-icon-view el-icon--right"/></el-button>
                  </el-col> -->
                </el-row>
...
export default {
  ...
  methods: {
    ...
    async goStep2() {
      if (this.BOMFile === '' || this.moduleFile === '' || this.CorrelationFile === '') {
        return this.$notify({
          message: '请上传文件~',
          type: 'warning',
          duration: 6000
        ...
        if (this.inputTabelListBOM.length !== 0 && this.inputTabelListModule.length !== 0 && this.inputTabelListCorrelation.length !== 0) {
          this.stepActive = 2
        } else {
          this.$notify({
            message: '请检查文件~',
