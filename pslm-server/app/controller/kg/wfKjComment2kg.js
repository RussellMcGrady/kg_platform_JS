...
async function wfKjComment2kg() {
    ...
    req.hUser = JSON.parse(req.hUser)
    req.hResource = JSON.parse(req.hResource)
    req.hComment = JSON.parse(req.hComment)
    ...
    var hUser = {}, hUp = {}, hUpTmp = {}, hComment = {}, hResource = {}, R1 = {}, R2 = {}, R3 = {}
    var name = 'kjzy'
    hUpTmp.id = req.hUser.id
    hUpTmp.label = req.hUser.label
    hUser[name] = req.hUser
    hUp[name] = hUpTmp
    hComment[name] = req.hComment
    hResource[name] = prepreocess(req.hResource)

    // var name1 = req.hUser.id + '_' + req.hComment.score
    // R1[name] = { label: '用户的评价', id: hUser[name].id + '_' + hResource[name].id }
    R1[name] = { label: '用户的评价' }

    // var name2 = req.hUser.id + '_' + hResource[name].title
    R2[name] = { label: '评价的资源' }

    // var name3 = hResource[name].title + '_' + req.hComment.score
    R3[name] = { label: '资源的评价' }

    // merge 结点
    var res0 = await (new Promise(function (resolve, reject) {
        graphdb.mergeOnEntity(hUser, hUp)
            .then(result => {
                const response0 = {
                    code: 20000,
    ...
    var res1 = await (new Promise(function (resolve, reject) {
        graphdb.mergeOnEntity(hComment, hComment)
            .then(result => {
                const response1 = {
                    code: 20000,
                    msg: 'database request success'
    ...
    var res2 = await (new Promise(function (resolve, reject) {
        graphdb.mergeOnEntity(hResource, hResource)
            .then(result => {
                const response2 = {
                    code: 20000,
                    msg: 'database request success'
    ...
    // merge 3条关系
    var res00 = await (new Promise(function (resolve, reject) {
        graphdb.mergeRelationshipBatchRel(hUser, R1, hComment)
            .then(result => {
                const response00 = {
                    code: 20000,
                    msg: 'database request success'
    ...
    }).then(function (response00) {
        console.log(response00)
        return JSON.stringify(response00)
    })
    )

    var res11 = await (new Promise(function (resolve, reject) {
        graphdb.mergeRelationshipBatchRel(hUser, R2, hResource)
            .then(result => {
                const response00 = {
                    code: 20000,
                    msg: 'database request success'
        ...
        return JSON.stringify(response11)
    })
    )
    ...
    var res = await (new Promise(function (resolve, reject) {
        graphdb.mergeRelationshipBatchRel(hResource, R3, hComment)
        .then(result => {
            const response22 = {
                code: 20000,
                msg: 'database request success'
        ...
        return JSON.stringify(response22)
    })
    )
    // set rel id for query
    // var res = await (new Promise(function (resolve, reject) {
    //     graphdb.mergeRelationshipBatchsetRel(hUser, R1, hComment)
    //         .then(result => {
